...
with tempfile.TemporaryDirectory(prefix='dogma-tests-') as td:
    for name in ('test_v566.py','test_v567.py','test_v568.py'):
        shutil.copyfile(root/'tests'/name,Path(td)/name)
    env=dict(os.environ,DOGMA_V566_MODULE=str(root/'dogma_semantic_v566.py'),DOGMA_V567_MODULE=str(root/'dogma_semantic_v567.py'),DOGMA_V568_MODULE=str(root/'dogma_semantic_v568.py'))
